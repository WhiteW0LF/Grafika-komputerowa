const paint = {
    ...
    mouseMove : function(e) {
        if (this.canDraw) {
            ...
            if (this.mode === 'circle') {
                this.ctx2.clearRect(0, 0, this.canvasElem2.width, this.canvasElem2.height);
                this.ctx2.beginPath();
                this.ctx2.arc(this.startX, this.startY, mousePos.x-this.startX, 0, Math.PI*2, true);
                this.ctx2.closePath();
                this.ctx2.stroke();
            }
            if (this.mode === 'triangle') {
                ...
                this.ctx2.moveTo(this.startX, this.startY);
                this.ctx2.lineTo(mousePos.x, mousePos.y);
                this.ctx2.lineTo(this.startX - (mousePos.x - this.startX), mousePos.y);
                this.ctx2.closePath();
                this.ctx2.stroke();
            }
            // if (this.mode === 'triangle') {
    ...
    init : function() {
        ...
        this.canvasBg.addEventListener('load', function() {
            ...
            this.sizeElem = document.querySelector('.paint_size');
            this.sizeElemVal = document.querySelector('.paint_size_val');
            this.sizeElemVal.innerText = this.sizeElem.value;

            //color piker
            this.colorElem = document.querySelector('.paint_color');
            ...
            //bind canvas elements
            this.bindElements();

            //reset canvas
            this.setupInitialCtx();
        }.bind(this));
        this.canvasBg.src = 'img/bg_img.jpg';
    }
};
